  ...
  >
    <header class="messages-route__chat-header">
      <div class="messages-route__chat-person">
        ...
        <div>
          <h1 id="message-detail-title">{{ conversation.participantName }}</h1>
    ...
    <div class="messages-route__history">
      ...
      <template v-else>
        ...
        <article
          ...
          :class="{ 'messages-route__message--mine': message.author === 'me' }"
        >
          <span
    ...
    <footer class="messages-route__composer">
      <button class="messages-route__composer-button" type="button" disabled>
...
import { useConversationActionMenu } from "../composables/useConversationActionMenu";
...
interface Props {
  conversation: MessageCenterConversation;
...
const {
  ...
  insertEmoji,
  toggleEmojiPicker,
} = useMessageComposer(props.thread.draft);
