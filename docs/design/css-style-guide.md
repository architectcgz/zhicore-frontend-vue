...
当前实际 UI 风格见 `docs/design/ui-style-baseline.md`。新增页面先判断页面类型，再选择容器和视觉密度：

| 页面类型                          | 推荐样式                                       | 不推荐样式                                   |
| --------------------------------- | ---------------------------------------------- | -------------------------------------------- |
| 首页 / 登录页 / 品牌表达页        | 可使用微光背景、玻璃面、强标题和少量品牌动效。 | 为了品牌感堆叠无真实信息的卡片。             |
| 发现 / 社区 / 资源 / 搜索等浏览页 | 平铺 header、分隔线、筛选控件、内容列表。      | 复刻首页 hero，或把每个 section 包成大卡片。 |
| 阅读 / 编辑 / 设置等任务页        | 工作台布局、稳定工具栏、真实状态反馈。         | 营销式首屏、解释性 UI 文案、动效抢主任务。   |

当 `brand-ui-design.md` 的概念图与当前代码风格冲突时，新增生产页面优先遵守 `ui-style-baseline.md` 和本文的 CSS owner 规则。
